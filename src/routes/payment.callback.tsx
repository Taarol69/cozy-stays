              ...
              {booking && (
                <div className="mt-6 grid gap-2 rounded-lg bg-muted/40 p-4 text-left text-sm">
                  ...
                  <div className="flex justify-between font-medium">
                    <span>Total</span>
                    <span className="text-gold">{formatNPR(Number(booking.total_price) * rate)} <span className="text-xs font-normal text-muted-foreground">(${Number(booking.total_price).toFixed(2)})</span></span>
                  </div>
                </div>
              )}
